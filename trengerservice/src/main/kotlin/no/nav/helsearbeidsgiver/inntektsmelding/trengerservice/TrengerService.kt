...
class TrengerService(private val rapidsConnection: RapidsConnection, override val redisStore: IRedisStore) : CompositeEventListener(redisStore) {
    ...
    override fun onError(feil: Fail): Transaction {
        ...
        if (utloesendeBehov == BehovType.HENT_TRENGER_IM) {
            ...
            redisStore.set(RedisKey.of(feil.transaksjonId.toString(), DataFelt.VIRKSOMHET), "Ukjent navn")
            ...
            redisStore.set(
                RedisKey.of(feil.transaksjonId.toString(), DataFelt.ARBEIDSTAKER_INFORMASJON),
                PersonDato("Ukjent navn", null, "").toJsonStr(PersonDato.serializer())
            )
            redisStore.set(
                RedisKey.of(feil.transaksjonId.toString(), DataFelt.ARBEIDSGIVER_INFORMASJON),
                PersonDato("Ukjent navn", null, "").toJsonStr(PersonDato.serializer())
            )
            ...
            redisStore.set(RedisKey.of(feil.transaksjonId.toString(), DataFelt.INNTEKT), UNDEFINED_FELT)
        }
        if (feilmelding != null) {
            val feilKey = RedisKey.of(feil.transaksjonId.toString(), feilmelding)
            val feilReport: FeilReport = redisStore.get(feilKey)?.fromJson(FeilReport.serializer()) ?: FeilReport()
                ...
                if (skjaeringstidspunkt != null) {
                    ...
                    "Fant ikke skjaeringstidspunkt å hente inntekt for.".also {
                        ...
                        val fail = Fail(
                            feilmelding = it,
                            event = event,
                            transaksjonId = uuid.let(UUID::fromString),
    ...
    override fun terminate(fail: Fail) {
        ...
        val clientId: String? = redisStore.get(RedisKey.of(fail.transaksjonId.toString(), fail.event))
        ...
        val feilReport: FeilReport = redisStore.get(RedisKey.of(uuid = fail.transaksjonId.toString(), Feilmelding("")))!!.fromJson(FeilReport.serializer())
        if (clientId != null) {
